...
class DeleteUserPresenter(val mListener:ValidateDataListener<UpdateAndDeleteUserModel>): KoinComponent {
    fun userData(info: UserIdAndPhoneModel){
        ...
        if (network.isNetworkAvailable(MainActivity.mContext)){
            ...
            apiServices
                .getApi()
                .deleteUser(MainActivity.mContext.usersUrl, info)
                .enqueue(object : Callback<UpdateAndDeleteUserModel> {
                    override fun onResponse(
                        call: Call<UpdateAndDeleteUserModel>,
                        response: Response<UpdateAndDeleteUserModel>
                    ) {
